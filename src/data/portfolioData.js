import { Code, Smartphone, Server, Database, Cloud, Workflow, BrainCircuit, Orbit, Network } from 'lucide-react';

export const skills = [
  { name: 'Python', icon:'https://img.icons8.com/?size=96&id=13441&format=png'},
  { name: 'Java', icon: 'https://img.icons8.com/?size=96&id=13679&format=png' },
  { name: 'Node.js', icon: 'https://img.icons8.com/?size=96&id=hsPbhkOH4FMe&format=png' },
  { name: 'React', icon: 'https://img.icons8.com/?size=100&id=35989&format=png' },
  { name: 'React Native', icon:'https://img.icons8.com/?size=96&id=123603&format=png' },
  { name: 'SQL', icon: 'https://www.ibm.com/adobe/dynamicmedia/deliver/dm-aid--f385b240-1636-41a3-97a6-a0a6fbc58352/azure-sql-server-monitoring.png?quality=90&preferwebp=true' },
  { name: 'AWS', icon: 'https://img.icons8.com/?size=96&id=33039&format=png' },
  { name: 'N8N', icon: 'https://upload.wikimedia.org/wikipedia/commons/5/53/N8n-logo-new.svg' },
  { name: 'Terraform', icon: 'https://img.icons8.com/?size=96&id=kEkT1u7zTDk5&format=png' },
  
];

export const projects = [
  {
    title: 'Desenvolvimento Mobile com React Native',
    description: 'Em desenvolvimento',
  },
  {
    title: 'Em breve',
    description: '',
  },
  {
    title: 'Em breve',
    description: '',
    
  }
];
